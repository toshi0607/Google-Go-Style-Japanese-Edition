...
Go標準ライブラリは、[`testing`パッケージ](https://pkg.go.dev/testing)を提供します。Googleのコードベースでは、Goのコードに許可されている唯一のテストフレームワークです。特に、[アサーションライブラリ](#アサーションライブラリ)やサードパーティのテストフレームワークは許可されていません。
